 ...
 * <p/>
 * In general, one would call {@link #getCategory()} to determine the type of
 * error that occurred, and act accordingly. The app can also call
...
public final class FacebookRequestError {
	...
	private FacebookRequestError(int requestStatusCode, int errorCode,
			int subErrorCode, String errorType, String errorMessage,
			JSONObject requestResultBody, JSONObject requestResult,
			Object batchRequestResult, HttpURLConnection connection,
		...
		// Initializes the error categories based on the documented error codes
		// as outlined here
		...
		if (isLocalException) {
			...
			if (errorCode == EC_UNKNOWN_ERROR
					|| errorCode == EC_SERVICE_UNAVAILABLE) {
				errorCategory = Category.SERVER;
			} else if (errorCode == EC_APP_TOO_MANY_CALLS
					|| errorCode == EC_USER_TOO_MANY_CALLS) {
				errorCategory = Category.THROTTLING;
			} else if (errorCode == EC_PERMISSION_DENIED
					|| EC_RANGE_PERMISSION.contains(errorCode)) {
			...
			} else if (errorCode == EC_INVALID_SESSION
					|| errorCode == EC_INVALID_TOKEN) {
				if (subErrorCode == EC_USER_CHECKPOINTED
						|| subErrorCode == EC_UNCONFIRMED_USER) {
	...
	private FacebookRequestError(int requestStatusCode, int errorCode,
			...
			JSONObject requestResultBody, JSONObject requestResult,
			Object batchRequestResult, HttpURLConnection connection) {
		this(requestStatusCode, errorCode, subErrorCode, errorType,
				errorMessage, requestResultBody, requestResult,
				batchRequestResult, connection, null);
	}
	...
	FacebookRequestError(HttpURLConnection connection, Exception exception) {
		this(
				...
				null,
				connection,
				(exception instanceof FacebookException) ? (FacebookException) exception
						: new FacebookException(exception));
	}

	public FacebookRequestError(int errorCode, String errorType,
			String errorMessage) {
		this(INVALID_HTTP_STATUS_CODE, errorCode, INVALID_ERROR_CODE,
				errorType, errorMessage, null, null, null, null, null);
	}

	/**
	 * Returns the resource id for a user-friendly message for the application
	...
	/**
	 * Returns whether direct user action is required to successfully continue
	 * with the Facebook operation. If user action is required, apps can also
	 * call {@link #getUserActionMessageId()} in order to get a resource id for
	...
	/**
	 * Returns the category in which the error belongs. Applications can use the
	 * category to determine how best to handle the errors (e.g. exponential
	 ...
	 * than using the {@link #getCategory()} method, but can provide further
	...
	/**
	 * Returns the full JSON response for the corresponding request. In a
	 * non-batch request, this would be the raw response in the form of a JSON
	 * object. In a batch request, this result will contain the body of the
	 * response as well as the HTTP headers that pertain to the specific request
	 * (in the form of a "headers" JSONArray).
	 * 
	...
	/**
	 * Returns the full JSON response for the batch request. If the request was
	 * not a batch request, then the result from this method is the same as
	 * {@link #getRequestResult()}. In case of a batch request, the result will
	 * be a JSONArray where the elements correspond to the requests in the
	 * batch. Callers should check the return type against either JSONObject or
	 * JSONArray and cast accordingly.
	 * 
	...
	public String toString() {
		return new StringBuilder("{HttpStatus: ").append(requestStatusCode)
	...
	static FacebookRequestError checkResponseAndCreateError(
			JSONObject singleResult, Object batchResult,
		...
		try {
			if (singleResult.has(CODE_KEY)) {
				...
				Object body = Utility.getStringPropertyAsJSON(singleResult,
						BODY_KEY, Response.NON_JSON_RESPONSE_PROPERTY);

				if (body != null && body instanceof JSONObject) {
					JSONObject jsonBody = (JSONObject) body;
					// Does this response represent an error from the service?
					// We might get either an "error"
					// with several sub-properties, or else one or more
					// top-level fields containing error info.
					...
					if (jsonBody.has(ERROR_KEY)) {
						...
						JSONObject error = (JSONObject) Utility
								.getStringPropertyAsJSON(jsonBody, ERROR_KEY,
										null);

						errorType = error.optString(ERROR_TYPE_FIELD_KEY, null);
						errorMessage = error.optString(ERROR_MESSAGE_FIELD_KEY,
								null);
						errorCode = error.optInt(ERROR_CODE_FIELD_KEY,
								INVALID_ERROR_CODE);
						errorSubCode = error.optInt(ERROR_SUB_CODE_KEY,
								INVALID_ERROR_CODE);
						hasError = true;
						...
						errorCode = jsonBody.optInt(ERROR_CODE_KEY,
								INVALID_ERROR_CODE);
						errorSubCode = jsonBody.optInt(ERROR_SUB_CODE_KEY,
								INVALID_ERROR_CODE);
						hasError = true;
					}
					...
					if (hasError) {
						return new FacebookRequestError(responseCode,
								errorCode, errorSubCode, errorType,
								errorMessage, jsonBody, singleResult,
								batchResult, connection);
					}
				}

				// If we didn't get error details, but we did get a failure
				// response code, report it.
				if (!HTTP_RANGE_SUCCESS.contains(responseCode)) {
					return new FacebookRequestError(
							...
							null,
							null,
							singleResult.has(BODY_KEY) ? (JSONObject) Utility
	...
	public enum Category {
		/**
		 * Indicates that the error is authentication related, and that the app
		 * should retry the request after some user action.
		 */
		AUTHENTICATION_RETRY,

		/**
		 * Indicates that the error is authentication related, and that the app
		 * should close the session and reopen it.
		 */
		...
		/**
		 * Indicates that the error implies the server had an unexpected failure
		 * or may be temporarily unavailable.
		 */
		SERVER,

		/**
		 * Indicates that the error results from the server throttling the
		 * client.
		 */
		THROTTLING,

		/**
		 * Indicates that the error is Facebook-related but cannot be
		 * categorized at this time, and is likely newer than the current
		 * version of the SDK.
		 */
		OTHER,

		/**
		 * Indicates that the error is an application error resulting in a bad
		 * or malformed request to the server.
		 */
		BAD_REQUEST,

		/**
		 * Indicates that this is a client-side error. Examples of this can
		 * include, but are not limited to, JSON parsing errors or
