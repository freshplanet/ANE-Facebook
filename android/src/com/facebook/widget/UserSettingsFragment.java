...
public class UserSettingsFragment extends FacebookFragment {
	...
	private Session userInfoSession; // the Session used to fetch the current
										// user info
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = inflater.inflate(
				AirFacebookExtension.getResourceId("layout.com_facebook_usersettingsfragment"), container, false);
		loginButton = (LoginButton) view
				.findViewById(AirFacebookExtension.getResourceId("id.com_facebook_usersettingsfragment_login_button"));
		loginButton.setProperties(loginButtonProperties);
		loginButton.setFragment(this);
		loginButton
				.setLoginLogoutEventName(AnalyticsEvents.EVENT_USER_SETTINGS_USAGE);
		...
		connectedStateLabel = (TextView) view
				.findViewById(AirFacebookExtension.getResourceId("id.com_facebook_usersettingsfragment_profile_name"));

		// if no background is set for some reason, then default to Facebook
		// blue
		if (view.getBackground() == null) {
			view.setBackgroundColor(getResources().getColor(
					AirFacebookExtension.getResourceId("color.com_facebook_blue")));
		} else {
	 ...
	 * <p/>
	 * Since the session can be automatically opened when the
	 * UserSettingsFragment is constructed, it's important to always pass in a
	 * consistent set of permissions to this method, or manage the setting of
	 * permissions outside of the LoginButton class altogether (by managing the
	 * session explicitly).
	 * 
	 * @param permissions
	...
	public void setReadPermissions(String... permissions) {
		loginButtonProperties.setReadPermissions(Arrays.asList(permissions),
				getSession());
	}
	...
	public void setPublishPermissions(String... permissions) {
		loginButtonProperties.setPublishPermissions(Arrays.asList(permissions),
				getSession());
	}
	...
	/**
	 * Returns the current OnErrorListener for this instance of
		...
		if (isSessionOpen()) {
			connectedStateLabel
					.setTextColor(getResources()
							.getColor(
									AirFacebookExtension.getResourceId("color.com_facebook_usersettingsfragment_connected_text_color")));
			connectedStateLabel
					.setShadowLayer(
							1f,
							0f,
							...
							getResources()
									.getColor(
											AirFacebookExtension.getResourceId("color.com_facebook_usersettingsfragment_connected_shadow_color")));

			if (user != null) {
				ImageRequest request = getImageRequest();
				if (request != null) {
					URI requestUrl = request.getImageUri();
					// Do we already have the right picture? If so, leave it
					// alone.
					if (!requestUrl.equals(connectedStateLabel.getTag())) {
						if (user.getId().equals(userProfilePicID)) {
							connectedStateLabel.setCompoundDrawables(null,
				...
				connectedStateLabel.setText(getResources().getString(
						AirFacebookExtension.getResourceId("string.com_facebook_usersettingsfragment_logged_in")));
				Drawable noProfilePic = getResources().getDrawable(
						AirFacebookExtension.getResourceId("drawable.com_facebook_profile_default_icon"));
				noProfilePic
						.setBounds(
								0,
								0,
								getResources()
										.getDimensionPixelSize(
												AirFacebookExtension.getResourceId("dimen.com_facebook_usersettingsfragment_profile_picture_width")),
								getResources()
										.getDimensionPixelSize(
												AirFacebookExtension.getResourceId("dimen.com_facebook_usersettingsfragment_profile_picture_height")));
				connectedStateLabel.setCompoundDrawables(null, noProfilePic,
						null, null);
			}
		} else {
			int textColor = getResources()
					.getColor(
							AirFacebookExtension.getResourceId("color.com_facebook_usersettingsfragment_not_connected_text_color"));
			connectedStateLabel.setTextColor(textColor);
			connectedStateLabel.setShadowLayer(0f, 0f, 0f, textColor);
			connectedStateLabel.setText(getResources().getString(
					AirFacebookExtension.getResourceId("string.com_facebook_usersettingsfragment_not_logged_in")));
	...
	private void processImageResponse(String id, ImageResponse response) {
		if (response != null) {
			...
			if (bitmap != null) {
				...
				userProfilePicID = id;
				connectedStateLabel.setCompoundDrawables(null, drawable, null,
						null);
